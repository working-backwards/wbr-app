...
- Connections YAML entries require `name`, `type` (`postgres`, `snowflake`, `athena`, `redshift`), and `config`. If a
  ...
  `AWS_REGION_NAME`). See the [Connections File](reference/connections.md) reference for full details.

## 4) Validation expectations

- `setup.week_ending` is mandatory and must use `DD-MMM-YYYY` (e.g., `25-SEP-2021`).
